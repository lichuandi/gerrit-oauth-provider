...
class CasOAuthService implements OAuthServiceProvider {
  ...
  public OAuthUserInfo getUserInfo(OAuthToken token) throws IOException {
    ...
    if (attrListJson == null) {
      throw new IOException(
          String.format("CAS response missing otherInfos: %s", response.getBody()));
    }

    String email = null, name = null, login = null;

    if (attrListJson.isJsonPrimitive()) {
      //isJsonPrimitive is String type
      // It is possible for CAS to be configured to not return any otherInfos (email, name, login),
      // in which case,
      // CAS returns an empty JSON object "otherInfos":{}, rather than "null" or an empty JSON array
      // on "otherInfos": []：yes  "otherInfos":{}
